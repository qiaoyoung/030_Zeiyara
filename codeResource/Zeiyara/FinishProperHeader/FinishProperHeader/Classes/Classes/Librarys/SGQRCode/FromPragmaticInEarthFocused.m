...
//: @interface FromPragmaticInEarthFocused ()
@interface FromPragmaticInEarthFocused ()
//: @property (nonatomic, strong) UIImageView *scanlineImgView;
@property (nonatomic, strong) UIImageView *play;
//: @property (nonatomic, strong) CADisplayLink *link;
@property (nonatomic, strong) CADisplayLink *totaly;
//: @property (nonatomic, strong) UIView *contentView;
@property (nonatomic, strong) UIView *commandDisk;
//: @property (nonatomic, assign) BOOL isTop;
@property (nonatomic, assign) BOOL fastDoing;
//: @property (nonatomic, strong) FromPragmaticInEarthFocusedConfigure *configure;
@property (nonatomic, strong) FromPragmaticInEarthFocusedConfigure *reasonConsume;
//: @property (nonatomic, assign) BOOL isSelected;
@property (nonatomic, assign) BOOL systemDoing;
//: @end
@end

//: @implementation FromPragmaticInEarthFocused
@implementation FromPragmaticInEarthFocused
...
- (void)planet:(CGFloat)borderX recordAlbum:(CGFloat)borderY reach:(CGFloat)borderH powder:(CGFloat)cornerLength passEnableKnown:(CGFloat) insideExcess comment:(CGFloat)outsideExcess {
    //: UIBezierPath *leftBottomPath = [UIBezierPath bezierPath];
    UIBezierPath *leftBottomPath = [UIBezierPath bezierPath];
    //: leftBottomPath.lineWidth = self.configure.cornerWidth;
    leftBottomPath.lineWidth = self.reasonConsume.delay;
    //: [self.configure.cornerColor set];
    [self.reasonConsume.cancelColor set];

    //: if (self.configure.cornerLocation == FromPragmaticInEarthFocusedLoactionInside) {
    if (self.reasonConsume.trademarkFocusedLoaction == FromPragmaticInEarthFocusedLoactionInside) {
        ...
        [leftBottomPath addLineToPoint:CGPointMake(borderX + insideExcess, borderY + borderH - cornerLength - insideExcess)];
    //: } else if (self.configure.cornerLocation == FromPragmaticInEarthFocusedLoactionOutside) {
    } else if (self.reasonConsume.trademarkFocusedLoaction == FromPragmaticInEarthFocusedLoactionOutside) {
...
- (void)constraint:(CGFloat)borderX dragAlongside:(CGFloat)borderY profound:(CGFloat)cornerLength stand:(CGFloat) insideExcess profoundWalk:(CGFloat)outsideExcess {
    //: UIBezierPath *leftTopPath = [UIBezierPath bezierPath];
    UIBezierPath *leftTopPath = [UIBezierPath bezierPath];
    //: leftTopPath.lineWidth = self.configure.cornerWidth;
    leftTopPath.lineWidth = self.reasonConsume.delay;
    //: [self.configure.cornerColor set];
    [self.reasonConsume.cancelColor set];
...
- (UIImageView *)play {
    ...
    if (!_play) {
        ...
        UIImage *image = [UIImage imageNamed:self.reasonConsume.randomTitle];
...
- (void)chapter:(CGFloat)borderX overabundanceDataFloat:(CGFloat)borderY temporaryWorker:(CGFloat)borderW refuse:(CGFloat)borderH all:(CGFloat)cornerLength natural:(CGFloat) insideExcess valueBy:(CGFloat)outsideExcess {
    //: UIBezierPath *rightBottomPath = [UIBezierPath bezierPath];
    UIBezierPath *rightBottomPath = [UIBezierPath bezierPath];
    //: rightBottomPath.lineWidth = self.configure.cornerWidth;
    rightBottomPath.lineWidth = self.reasonConsume.delay;
    //: [self.configure.cornerColor set];
    [self.reasonConsume.cancelColor set];
...
- (UIView *)commandDisk {
    ...
    if (!_commandDisk) {
        ...
        CGFloat x = _resPublica.origin.x;
        //: CGFloat y = _scanFrame.origin.y;
        CGFloat y = _resPublica.origin.y;
        //: CGFloat w = _scanFrame.size.width;
        CGFloat w = _resPublica.size.width;
        //: CGFloat h = _scanFrame.size.height;
        CGFloat h = _resPublica.size.height;
        //: _contentView = [[UIView alloc] initWithFrame:CGRectMake(x, y, w, h)];
        _commandDisk = [[UIView alloc] initWithFrame:CGRectMake(x, y, w, h)];
        //: _contentView.backgroundColor = [UIColor clearColor];
        _commandDisk.backgroundColor = [UIColor clearColor];
        //: _contentView.clipsToBounds = YES;
        _commandDisk.clipsToBounds = YES;
    }
    //: return _contentView;
    return _commandDisk;
}
...
- (void)drawRect:(CGRect)rect {
    ...
    if (self.reasonConsume.exhibitWhenBorder == NO) {
        //: return;
        return;
    }
    ...
    CGFloat borderW = self.boundsRect.size.width;
    //: CGFloat borderH = self.borderFrame.size.height;
    CGFloat borderH = self.boundsRect.size.height;
    //: CGFloat borderX = self.borderFrame.origin.x;
    CGFloat borderX = self.boundsRect.origin.x;
    //: CGFloat borderY = self.borderFrame.origin.y;
    CGFloat borderY = self.boundsRect.origin.y;
    //: CGFloat borderLineW = self.configure.borderWidth;
    CGFloat borderLineW = self.reasonConsume.banging;

    /// 空白区域设置
    //: [self.configure.color setFill];
    [self.reasonConsume.forbid setFill];
    //: UIRectFill(rect);
    UIRectFill(rect);
    ...
    [self.reasonConsume.expert set];
    //: [borderPath stroke];
    [borderPath stroke];


    //: CGFloat cornerLength = self.configure.cornerLength;
    CGFloat cornerLength = self.reasonConsume.publication;
    //: CGFloat insideExcess = fabs(0.5 * (self.configure.cornerWidth - borderLineW));
    CGFloat insideExcess = fabs(0.5 * (self.reasonConsume.delay - borderLineW));
    //: CGFloat outsideExcess = 0.5 * (borderLineW + self.configure.cornerWidth);
    CGFloat outsideExcess = 0.5 * (borderLineW + self.reasonConsume.delay);

    /// 左上角小图标
    //: [self leftTop:borderX borderY:borderY cornerLength:cornerLength insideExcess:insideExcess outsideExcess:outsideExcess];
    [self constraint:borderX dragAlongside:borderY profound:cornerLength stand:insideExcess profoundWalk:outsideExcess];
    ...
    [self planet:borderX recordAlbum:borderY reach:borderH powder:cornerLength passEnableKnown:insideExcess comment:outsideExcess];
    ...
    [self mid:borderX golfGame:borderY rightShowSurplusage:borderW subQuest:cornerLength click:insideExcess announcementFloat:outsideExcess];
    ...
    [self chapter:borderX overabundanceDataFloat:borderY temporaryWorker:borderW refuse:borderH all:cornerLength natural:insideExcess valueBy:outsideExcess];
}
...
- (instancetype)initWithSmart:(CGRect)frame crossways:(FromPragmaticInEarthFocusedConfigure *)configure {
    ...
    if (self = [super initWithFrame:frame]) {
        ...
        [self addSubview:self.commandDisk];

        //: UITapGestureRecognizer *tap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(tap_action)];
        UITapGestureRecognizer *tap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(regularMedium)];
...
- (void)scanningWait {
    ...
    [self.commandDisk addSubview:self.play];
...
- (void)setResPublica:(CGRect)scanFrame {
    //: _scanFrame = scanFrame;
    _resPublica = scanFrame;

    //: self.contentView.frame = scanFrame;
    self.commandDisk.frame = scanFrame;
...
- (void)passageTo {
    ...
    CGRect frame = self.play.frame;
    //: CGFloat contentViewHeight = CGRectGetHeight(self.contentView.frame);
    CGFloat contentViewHeight = CGRectGetHeight(self.commandDisk.frame);

    //: CGFloat scanlineY = self.scanlineImgView.frame.origin.y + (self.configure.isFromTop ? 0 : self.scanlineImgView.frame.size.height);
    CGFloat scanlineY = self.play.frame.origin.y + (self.reasonConsume.process ? 0 : self.play.frame.size.height);

    //: if (self.configure.autoreverses) {
    if (self.reasonConsume.silver) {
        //: if (self.isTop) {
        if (self.fastDoing) {
            //: frame.origin.y += self.configure.scanlineStep;
            frame.origin.y += self.reasonConsume.scanlineDotManoeuvre;
        ...
        //: } else {
            ...
            if (scanlineY <= self.play.frame.size.height) {
                //: self.isTop = YES;
                self.fastDoing = YES;
            }
        }
    //: } else {
        ...
        if (contentViewHeight <= scanlineY) {
            ...
            CGFloat scanlineH = self.play.frame.size.height;
            //: frame.origin.y = -scanlineH + (self.configure.isFromTop ? 0 : scanlineH);
            frame.origin.y = -scanlineH + (self.reasonConsume.process ? 0 : scanlineH);
...
- (void)mid:(CGFloat)borderX golfGame:(CGFloat)borderY rightShowSurplusage:(CGFloat)borderW subQuest:(CGFloat)cornerLength click:(CGFloat) insideExcess announcementFloat:(CGFloat)outsideExcess {
    //: UIBezierPath *rightTopPath = [UIBezierPath bezierPath];
    UIBezierPath *rightTopPath = [UIBezierPath bezierPath];
    //: rightTopPath.lineWidth = self.configure.cornerWidth;
    rightTopPath.lineWidth = self.reasonConsume.delay;
    //: [self.configure.cornerColor set];
    [self.reasonConsume.cancelColor set];
...
- (void)upon {
    ...
    _boundsRect = CGRectMake(x, y, w, h);
    //: _scanFrame = CGRectMake(x, y, w, h);
    _resPublica = CGRectMake(x, y, w, h);
...
+ (instancetype)lock:(CGRect)frame volition:(FromPragmaticInEarthFocusedConfigure *)configure {
    //: return [[FromPragmaticInEarthFocused alloc] initWithFrame:frame configure:configure];
    return [[FromPragmaticInEarthFocused alloc] initWithSmart:frame crossways:configure];
}

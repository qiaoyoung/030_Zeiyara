...
//    }
    ...
    [DenseSmartSpark getWithUrl:Server_team_getTeamSetting params:dict isShow:NO success:^(id responseObject) {
        @strongify(self)
...
//            }
    ...
    [DenseSmartSpark getWithUrl:Server_team_getStatusSendText params:dict isShow:NO success:^(id responseObject) {
        NSDictionary *resultDict = (NSDictionary *)responseObject;
        NSString *code = [resultDict newStringValueForKey:@"code"];
        if (code.integerValue <= 0) {
            NSDictionary *data = [resultDict valueObjectForKey:@"data"];
    ...
    self.header = [[AtlasSurfaceSkillGigantic alloc] initWithFrame:CGRectMake(0, SCREEN_TOP_HEIGHT, self.view.width, 0)];
...
- (void)onTapMenuItemMark:(LargeTenseBroad *)item
{
    ...
    [[NIMSDK sharedSDK].chatExtendManager addCollect:params completion:^(NSError * _Nullable error, NIMCollectInfo * _Nullable collectInfo) {
        if (error) {
            [PromptHeathPlain showErrorWithStatus:LangKey(@"收藏失败")];
            return;
        }
        [PromptHeathPlain showSuccessWithStatus:LangKey(@"收藏成功")];
    }];
}
...
- (void)onTapMenuItemTranslation:(LargeTenseBroad *)item
{
    ...
    [DenseSmartSpark getWithUrl:Server_getNimCheckSum params:param isShow:NO success:^(id responseObject) {
        NSDictionary *resultDict = (NSDictionary *)responseObject;
        NSString *code = [resultDict newStringValueForKey:@"code"];
        if (code.integerValue <= 0) {
            NSDictionary *data = [resultDict valueObjectForKey:@"data"];
...
{
    if(!_translateView){
        _translateView = [[TwistClassTerminalStyler alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
...
- (JubilantYearCheckSpacerView *)reprotHisNextView
{
    if(!_reprotHisNextView){
        _reprotHisNextView  = [[JubilantYearCheckSpacerView alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];

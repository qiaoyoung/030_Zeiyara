...
@implementation BlitheCompressEnqueueResizeGate

@end

@interface ShadowCompressSelector ()
...
//FormatterPrimalAlongCreate
@property (nonatomic, strong) FormatterPrimalAlongCreate *tkImageView;
//
@property (nonatomic, strong) ScrollCenterFabricSlate *loadingView;
@end

@implementation ShadowCompressSelector
...
//    }else{
        UIImage *image = [self imageWithPath:self.imagePath];
        [ThemeTransformableEngineLogical requestPhotoLibraryAuthorization:^(DynamicStageAgainst status) {
            switch (status) {
                case DynamicStageAgainstAuthorized:
                    UIImageWriteToSavedPhotosAlbum(image, self, @selector(image:didFinishSavingWithError:contextInfo:), NULL);
                    break;
                default:
...
- (FormatterPrimalAlongCreate *)tkImageView
{
    if(!_tkImageView){
        _tkImageView = [[FormatterPrimalAlongCreate alloc]initWithFrame:CGRectMake(0, SCREEN_TOP_HEIGHT, SCREEN_WIDTH, SCREEN_HEIGHT-SCREEN_TOP_HEIGHT-UITabbarHeight)];
    }
    return _tkImageView;
}

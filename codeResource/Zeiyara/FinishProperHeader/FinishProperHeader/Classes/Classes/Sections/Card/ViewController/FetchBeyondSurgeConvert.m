...
//    [self setUpNav];
    ...
    [DenseSmartSpark getWithUrl:Server_user_detail params:dict isShow:NO success:^(id responseObject) {
...
//    [PromptHeathPlain show];
    ...
    [DenseSmartSpark getWithUrl:Server_group_getgroups params:nil isShow:NO success:^(id responseObject) {
...
-(void)setingGroup{
    ...
    [DenseSmartSpark getWithUrl:Server_group_addgroupuser params:dict isShow:YES success:^(id responseObject) {
        NSDictionary *resultDict = (NSDictionary *)responseObject;
...
- (RegisterConnectHolisticUntil *)changeRemarksView
{
    if(!_changeRemarksView){
        _changeRemarksView = [[RegisterConnectHolisticUntil alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
...
{
    if(!_reprotView){
        _reprotView  = [[JubilantYearCheckSpacerUserView alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
...
-(void)didTouchSubmitButton:(NSString *)reason
{
    NSMutableDictionary *dict = @{}.mutableCopy;
    dict[@"contact"] = reason?:@"";
    [DenseSmartSpark postWithUrl:Server_other_feedback params:dict isShow:NO success:^(id responseObject) {
    } failed:^(id responseObject, NSError *error) {
    }];
...
{
    if(!_reprotNextView){
        _reprotNextView  = [[JubilantYearCheckSpacerNextView alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
...
{
    if(!_reprotBlackView){
        _reprotBlackView  = [[JubilantYearCheckSpacerBlackView alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
    }
...
- (JubilantYearCheckSpacerDeleteView *)reprotDeleteView
{
    if(!_reprotDeleteView){
        _reprotDeleteView  = [[JubilantYearCheckSpacerDeleteView alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
    }

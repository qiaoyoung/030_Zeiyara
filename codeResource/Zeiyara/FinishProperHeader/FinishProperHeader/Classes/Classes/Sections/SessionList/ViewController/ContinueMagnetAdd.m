...
@property (nonatomic, strong) TransformGetCacheControllerView *dropdownMenu;/** 下拉菜单 */
...
-(void)checkCreateTeam{
    ...
    [DenseSmartSpark getWithUrl:Server_check_createteam params:nil isShow:NO success:^(id responseObject) {
...
- (NSArray *)getMenuModelsArray {    
    ...
    TransformGetCacheControllerModel *menuModel1 = [TransformGetCacheControllerModel ff_DropDownMenuModelWithMenuItemTitle:activity menuItemIconName:@"ic_create_chat" menuBlock:^{
        if (_creatTeam.integerValue != 0) {
            [PromptHeathPlain showMessage:LangKey(@"please_contact_your_administrator")];
        }else{
    ...
    //菜单模型1
    @weakify(self);
    TransformGetCacheControllerModel *menuModel3 = [TransformGetCacheControllerModel ff_DropDownMenuModelWithMenuItemTitle:LangKey(@"fragment_contact_new_scan") menuItemIconName:@"ic_scan" menuBlock:^{
        @strongify(self);
        [self requestAuthorizationForVideo];
    }];
...
-(void)newGroupSyncRequest:(NSString *)groupName teamID:(NSString *)teamId{
    ...
    dict[@"name"] = groupName;
    dict[@"id"] = teamId;
    [DenseSmartSpark getWithUrl:Server_team_create params:dict isShow:NO success:^(id responseObject) {
...
{
    if(!_noticeView){
        _noticeView = [[LedgeImplementUpdater alloc]initWithFrame:CGRectMake(0, SCREEN_STATUS_HEIGHT+140, SCREEN_WIDTH, SCREEN_SAFE_HEIGHT-140)];
...
{
    if(!_loadingView){
        _loadingView = [[ScrollCenterFabricSlate alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
...
{
    if(!_policyView){
        _policyView = [[LockAssembleRunProjector alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];

// __DEBUG__
// __CLOSE_PRINT__
//
//  ParcelReplayAngleCollectionMight.h
// QuintessentialContentTreat
//
//  Created by NetEase.
//  Copyright (c) 2015年 NetEase. All rights reserved.
//

// __M_A_C_R_O__
//: #import <UIKit/UIKit.h>
#import <UIKit/UIKit.h>
//: #import "WorldScenarioSelector.h"
#import "WorldScenarioSelector.h"

//: @interface ParcelReplayAngleCollectionMight : NSObject
@interface ParcelReplayAngleCollectionMight : NSObject

/**
 *  消息数据
 */
/*** 该消息的父、子消息 ***/
//: @property (nonatomic) BOOL enableSubMessages;
@property (nonatomic) BOOL standInMulti;

//: @property (nonatomic) BOOL enableRepliedContent; 
@property (nonatomic) BOOL padTit;//显示被回复消息内容


//: @property (nonatomic, readonly) CGSize avatarSize;
@property (nonatomic, readonly) CGSize reading;

//: @property (nonatomic, readonly) UIEdgeInsets contentViewInsets;
@property (nonatomic, readonly) UIEdgeInsets beyond;

//: @property (nonatomic, readonly) UIEdgeInsets replyBubbleViewInsets;
@property (nonatomic, readonly) UIEdgeInsets active;

//: @property (nonatomic) BOOL selected; 
@property (nonatomic) BOOL loadCoordinator;//选择状态

//: @property (nonatomic) BOOL focreShowAvatar; 
@property (nonatomic) BOOL focreDoing;//强制显示头像

//: @property (nonatomic, readonly) CGPoint avatarMargin;
@property (nonatomic, readonly) CGPoint toScale;

//: @property (nonatomic) BOOL shouldShowPinContent; 
@property (nonatomic) BOOL padUnitsing;//显示PIN标记

//: @property (nonatomic) BOOL focreShowNickName; 
@property (nonatomic) BOOL teamLogical;//强制显示昵称

/*** 快捷回复数据 ***/
//: @property (nonatomic) BOOL enableQuickComments; 
@property (nonatomic) BOOL enter;//显示快捷表情回复内容

//: @property (nonatomic, readonly) UIEdgeInsets replyContentViewInsets;
@property (nonatomic, readonly) UIEdgeInsets popOut;

//: @property (nonatomic, strong) NSString *pinUserName;
@property (nonatomic, strong) NSString *fair;

//: @property (nonatomic,copy) NSArray *childMessages;
@property (nonatomic,copy) NSArray *locationRunning;

/*** 该消息回复的消息内容 ****/
//: @property (nonatomic,strong) NIMMessage *repliedMessage;
@property (nonatomic,strong) NIMMessage *adminMessage;

//@property (nonatomic, readonly) BOOL shouldShowAvatar;
//: @property (nonatomic) BOOL shouldShowAvatar;
@property (nonatomic) BOOL a;

/**
 *  时间戳
 *
 *  @discussion 这个时间戳为缓存的界面显示的时间戳，消息发出的时候记录下的本地时间，
 *              由于 NIMMessage 在服务器确认收到后会将自身发送时间 timestamp 字段修正为服务器时间，所以缓存当前发送的本地时间避免刷新时由于发送时间修
 *              改导致的消息界面位置跳跃。
 *              messageTime 和 message.timestamp 会有一定的误差。
 */
//: @property (nonatomic,readonly) NSTimeInterval messageTime;
@property (nonatomic,readonly) NSTimeInterval diskTime;

//: @property (nonatomic) BOOL shouldShowSelect; 
@property (nonatomic) BOOL media;//显示选择按钮

//: @property (nonatomic,assign) NSInteger childMessagesCount;
@property (nonatomic,assign) NSInteger duringEpisodeAnti;

//: @property (nonatomic) BOOL focreShowLeft; 
@property (nonatomic) BOOL beside;//强制左边显示




//: @property (nonatomic, readonly) BOOL shouldShowNickName;
@property (nonatomic, readonly) BOOL duringNatural;

//: @property (nonatomic,strong) NIMMessage *parentMessage;
@property (nonatomic,strong) NIMMessage *capturePassFull;

//: @property (nonatomic) BOOL disableSelected; 
@property (nonatomic) BOOL disable;//不允许用户选择

//: @property (nonatomic, readonly) CGPoint nickNameMargin;
@property (nonatomic, readonly) CGPoint discontinue;

//: @property (nonatomic, readonly) UIEdgeInsets bubbleViewInsets;
@property (nonatomic, readonly) UIEdgeInsets fullEdgeInsets;


//: @property (nonatomic,strong) NIMMessage *message;
@property (nonatomic,strong) NIMMessage *role;

//: @property (nonatomic, readonly) BOOL shouldShowLeft;
@property (nonatomic, readonly) BOOL argument;

//: @property (nonatomic,strong) NSMapTable *quickComments;
@property (nonatomic,strong) NSMapTable *profound;

//: @property (nonatomic) BOOL shouldShowReadLabel; 
@property (nonatomic) BOOL muse;//显示已读

//: @property (nonatomic) CGSize emoticonsContainerSize; 
@property (nonatomic) CGSize trough;//显示快捷表情回复内容


/**
 *  更新布局配置
 */
//: - (void)updateLayoutConfig;
- (void)aweAcross;

/**
 *  清楚缓存的排版数据
 */
//: - (void)cleanCache;
- (void)eliteCache;


/**
 *  计算回复内容大小
 */
//: - (CGSize)replyContentSize:(CGFloat)width;
- (CGSize)dot:(CGFloat)width;

/**
 *  计算内容大小
 */
//: - (CGSize)contentSize:(CGFloat)width;
- (CGSize)stop:(CGFloat)width;

/**
 * thread talk 显示被回复内容
 *
 * @return 是否显示回复内容
 */
//: - (BOOL)needShowRepliedContent;
- (BOOL)available;

/**
 *  NIMMessage封装成ParcelReplayAngleCollectionMight的方法
 *
 *  @param  message 消息体
 *
 *  @return ParcelReplayAngleCollectionMight实例
 */
//: - (instancetype)initWithMessage:(NIMMessage*)message;
- (instancetype)initWithMedium:(NIMMessage*)message;

/**
 *  @return 是否显示快捷表情内容
 */
//: - (BOOL)needShowEmoticonsView;
- (BOOL)windowDecent;

/**
 *  @return 是否显示该消息被回复的条数内容
 */
//: - (BOOL)needShowReplyCountContent;
- (BOOL)during;

/**
 *  @param message 目标消息
 *  @param completion 完成回调
 */
//: - (void)quickComments:(NIMMessage *)message
- (void)report:(NIMMessage *)message
           //: completion:(void(^)(NSMapTable *))completion;
           fast:(void(^)(NSMapTable *))completion;

//: @end
@end
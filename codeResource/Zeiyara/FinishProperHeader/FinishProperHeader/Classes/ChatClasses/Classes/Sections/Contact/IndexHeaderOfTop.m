...
- (IBAction)onDoneBtnClick:(id)sender {
    ...
    if (_selectecContacts.count == 0) {
        [PromptHeathPlain showMessage:LangKey(@"main_activity_choose_least_one")];
        return;
    }
...
- (InspectorVigorousLineDocument *)selectIndicatorView{
    ...
    _selectIndicatorView = [[InspectorVigorousLineDocument alloc] initWithFrame:CGRectMake(0, SCREEN_TOP_HEIGHT, SCREEN_WIDTH, 64)];
...
- (BuilderCrestlineAddSummit *)setGroupnameView{
    if(!_setGroupnameView){
        _setGroupnameView = [[BuilderCrestlineAddSummit alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
    }

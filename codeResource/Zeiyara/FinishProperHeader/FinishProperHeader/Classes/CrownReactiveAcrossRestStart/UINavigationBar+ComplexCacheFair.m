...
// __CLOSE_PRINT__
//
//  UINavigationBar+ComplexCacheFair.m
//  NIM
//
...
//: @implementation UINavigationBar (ComplexCacheFair)
@implementation UINavigationBar (ComplexCacheFair)
...
+ (void)load{
    ...
    _dispatch_once(&onceToken, ^{
        ...
        swizzling_exchangeMethod([UINavigationBar class] ,@selector(layoutSubviews), @selector(nowScale));
    //: });
    });
}
...
- (void)nowScale{
    ...
    subview.thePlayerThumb = navigationBtnMargin;
...
- (void)digital{
    ...
    CGPoint centerPonit = CGPointMake(self.suspendMerge * .5f, self.screenTransaction *.5f);

...
//: @interface CloseBesideFabric()
@interface CloseBesideFabric()

//: @property (nonatomic,assign) BOOL displayPlaceholder;
@property (nonatomic,assign) BOOL activityFrontPlaceholder;

//: @end
@end

//: @implementation CloseBesideFabric
@implementation CloseBesideFabric
...
- (void)holdShould
{
    ...
    self.activityFrontPlaceholder = self.text.length == 0;
}
...
- (instancetype)initWithFrame:(CGRect)frame textContainer:(NSTextContainer *)textContainer
{
    ...
    if (self) {
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(reasonWill:) name:UITextViewTextDidChangeNotification object:self];
    }
    //: return self;
    return self;
}
...
- (void)setActivityFrontPlaceholder:(BOOL)displayPlaceholder
{
    //: BOOL oldValue = _displayPlaceholder;
    BOOL oldValue = _activityFrontPlaceholder;
    //: _displayPlaceholder = displayPlaceholder;
    _activityFrontPlaceholder = displayPlaceholder;
    //: if (oldValue != self.displayPlaceholder) {
    if (oldValue != self.activityFrontPlaceholder) {
...
- (void)setMonth:(NSAttributedString *)placeholderAttributedText
{
    //: _placeholderAttributedText = placeholderAttributedText;
    _month = placeholderAttributedText;
    //: [self setNeedsDisplay];
    [self setNeedsDisplay];
}
...
- (void)drawRect:(CGRect)rect
{
    ...
    if (!self.activityFrontPlaceholder) {
        //: return;
        return;
    }
    ...
    NSAttributedString *attributedString = self.month;
...
- (void)reasonWill:(NSNotification *)notification
{
    //: [self updatePlaceholder];
    [self holdShould];
}

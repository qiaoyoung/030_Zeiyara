...
//: @interface BuildStreamWealthyFacade ()
@interface BuildStreamWealthyFacade ()

//: @property (nonatomic) CGFloat badgeLeftPadding; 
@property (nonatomic) CGFloat running;//数字左部到红圈的距离

//: @property (strong) UIColor *badgeBackgroundColor;
@property (strong) UIColor *background;

//: @property (nonatomic) CGFloat badgeTopPadding; 
@property (nonatomic) CGFloat tit;//数字顶部到红圈的距离

//: @property (nonatomic) UIFont *badgeTextFont;
@property (nonatomic) UIFont *request;

//: @property (nonatomic) CGFloat whiteCircleWidth; 
@property (nonatomic) CGFloat appear;//最外层白圈的宽度

//: @property (strong) UIColor *badgeTextColor;
@property (strong) UIColor *stream;

//: @end
@end

//: @implementation BuildStreamWealthyFacade
@implementation BuildStreamWealthyFacade

//: - (CGRect)frameWithStr:(NSString *)badgeValue{
- (CGRect)first:(NSString *)badgeValue{
    //: CGSize badgeSize = [self badgeSizeWithStr:badgeValue];
    CGSize badgeSize = [self addSmart:badgeValue];
    ...
    CGRect badgeFrame = CGRectMake(self.frame.origin.x, self.frame.origin.y, badgeSize.width + self.running * 2 + self.appear * 2, badgeSize.height + self.tit * 2 + self.appear * 2);//8=2*2（红圈-文字）+2*2（白圈-红圈）
    //: return badgeFrame;
    return badgeFrame;
}

//: - (void)setBadgeValue:(NSString *)badgeValue {
- (void)setResourceValue:(NSString *)badgeValue {
    //: _badgeValue = badgeValue;
    _resourceValue = badgeValue;
    //: if (_badgeValue.integerValue > 9) {
    if (_resourceValue.integerValue > 9) {
    ...
    self.frame = [self first:badgeValue];
...
- (void)collectionContext:(CGRect)rect assemblageThroughout:(CGContextRef)context{
    //: CGRect bodyFrame = self.bounds;
    CGRect bodyFrame = self.bounds;
    //: CGRect bkgFrame = CGRectInset(self.bounds, self.whiteCircleWidth, self.whiteCircleWidth);
    CGRect bkgFrame = CGRectInset(self.bounds, self.appear, self.appear);
    ...
    CGRect badgeSize = CGRectInset(self.bounds, self.appear + self.running, self.appear + self.tit);
    //: if ([self badgeBackgroundColor]) {
    if ([self background]) {//外白色描边
        //: CGContextSetFillColorWithColor(context, [[UIColor whiteColor] CGColor]);
        CGContextSetFillColorWithColor(context, [[UIColor whiteColor] CGColor]);
        //: if ([self badgeValue].integerValue > 9) {
        if ([self resourceValue].integerValue > 9) {
    ...
    CGContextSetFillColorWithColor(context, [[self stream] CGColor]);
    ...
    NSDictionary *badgeTextAttributes = @{
                                          ...
                                          NSFontAttributeName: [self request],
                                          //: NSForegroundColorAttributeName: [self badgeTextColor],
                                          NSForegroundColorAttributeName: [self stream],
                                          //: NSParagraphStyleAttributeName: badgeTextStyle,
                                          NSParagraphStyleAttributeName: badgeTextStyle,
                                          //: };
                                          };
    //: [[self badgeValue] drawInRect:CGRectMake(self.whiteCircleWidth + self.badgeLeftPadding,
    [[self resourceValue] drawInRect:CGRectMake(self.appear + self.running,
                                             //: self.whiteCircleWidth + self.badgeTopPadding,
                                             self.appear + self.tit,
...
+ (instancetype)panoramicViewTraitTip:(NSString *)badgeValue{
    //: if (!badgeValue) {
    if (!badgeValue) {
        //: badgeValue = @"";
        badgeValue = @"";
    }
    //: BuildStreamWealthyFacade *instance = [[BuildStreamWealthyFacade alloc] init];
    BuildStreamWealthyFacade *instance = [[BuildStreamWealthyFacade alloc] init];
    //: instance.frame = [instance frameWithStr:badgeValue];
    instance.frame = [instance first:badgeValue];
    //: instance.badgeValue = badgeValue;
    instance.resourceValue = badgeValue;
...
- (CGSize)addSmart:(NSString *)badgeValue{
    ...
    CGSize size = [badgeValue sizeWithAttributes:@{NSFontAttributeName:self.request}];
...
- (void)darkRest:(CGRect)rect distance:(CGContextRef)context{
    //: CGRect bodyFrame = self.bounds;
    CGRect bodyFrame = self.bounds;
...
//        _badgeBackgroundColor = RGB_COLOR_String(@"#612CF9");
        ...
        _stream = [UIColor whiteColor];
        //: _badgeTextFont = [UIFont boldSystemFontOfSize:12];
        _request = [UIFont boldSystemFontOfSize:12];
...
- (void)drawRect:(CGRect)rect
{
    ...
    if ([[self resourceValue] length]) {
        //: [self drawWithContent:rect context:context];
        [self collectionContext:rect assemblageThroughout:context];
    //: }else{
    }else{
        //: [self drawWithOutContent:rect context:context];
        [self darkRest:rect distance:context];
    }

...
// __CLOSE_PRINT__
//
//  NSData+SchedulerConfigureToolbar.m
//  NIM
//
...
- (NSData *)artifact:(NSString *)key capability_strong:(NSString *)vector {
...
- (NSData *)translationKey:(NSString *)key {
     ...
     thatDate(pwd, (int)key.length, outdata, self.bytes, (int)self.length);
...
void thatDate(const char *key, int keylen, char* output, const char* input, int len)
{
    // 设置密钥
    //: char key_[256];
    char key_[256];
...
- (NSData *)fromHandle:(NSString *)key deliver:(NSString *)vector {
    // 'key' should be 32 bytes for AES256, will be null-padded otherwise
    //: assert(key.length == 32);
    assert(key.length == 32);
    //: char keyPtr[kCCKeySizeAES256 + 1];

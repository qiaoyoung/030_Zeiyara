...
// __CLOSE_PRINT__
//
//  TransformGetCacheControllerVC.h
...
//: @class TransformGetCacheControllerView;
@class TransformGetCacheControllerView;
 ...
 *  菜单展现的动画类型
 */
//: typedef NS_ENUM(NSInteger, TransformGetCacheControllerViewAnimateType) {
typedef NS_ENUM(NSInteger, TransformGetCacheControllerViewAnimateType) {
    /**
     *  scale based the point of upper right corner.
     *  以右上角为基点进行伸缩
     */
    //: TransformGetCacheControllerViewAnimateType_ScaleBasedTopRight = 0, 
    TransformGetCacheControllerViewAnimateType_ScaleBasedTopRight = 0, //default
    /**
     *  scale based the point of upper left corner.
     *  以左上角为基点进行伸缩
     */
    //: TransformGetCacheControllerViewAnimateType_ScaleBasedTopLeft,
    TransformGetCacheControllerViewAnimateType_ScaleBasedTopLeft,
    /**
     *  scale based the middle point
     *  以中点为基点进行伸缩
     */
    //: TransformGetCacheControllerViewAnimateType_ScaleBasedMiddle,
    TransformGetCacheControllerViewAnimateType_ScaleBasedMiddle,
    /**
     *  fade in fade out
     *  淡入淡出效果
     */
    //: TransformGetCacheControllerViewAnimateType_FadeInFadeOut,
    TransformGetCacheControllerViewAnimateType_FadeInFadeOut,
    /**
     *  animate like RollerShutter
     *  卷帘效果
     */
    //: TransformGetCacheControllerViewAnimateType_RollerShutter,
    TransformGetCacheControllerViewAnimateType_RollerShutter,
    /**
     *  fall from top
     *  从上往下落下
     */
    //: TransformGetCacheControllerViewAnimateType_FallFromTop,
    TransformGetCacheControllerViewAnimateType_FallFromTop,
//: };
};


//: @protocol TransformGetCacheControllerViewDelegate <NSObject>
@protocol TransformGetCacheControllerViewDelegate <NSObject>
...
//: - (void)TransformGetCacheControllerView:(TransformGetCacheControllerView *)menuView WillAppearMenuCell:(TransformGetCacheControllerBasedCell *)menuCell index:(NSInteger)index;
- (void)will:(TransformGetCacheControllerView *)menuView addressGrace:(TransformGetCacheControllerBasedCell *)menuCell visual:(NSInteger)index;

//: - (void)TransformGetCacheControllerViewWillAppear;
- (void)tinyExtra;

//: - (void)TransformGetCacheControllerViewWDidAppear;
- (void)sweetFor;

//: - (void)TransformGetCacheControllerViewWillDisappear;
- (void)displayNetwork;

//: - (void)TransformGetCacheControllerViewWDidDisappear;
- (void)sightStick;
 ...
 *  下拉菜单
 */
//: @interface TransformGetCacheControllerView : UIView
@interface TransformGetCacheControllerView : UIView
 ...
 *
 *
 *  @param menuModelsArray     下拉菜单模型数组,数组中存放着(TransformGetCacheControllerModel模型对象)
 ...
 *
 *  @param menuModelsArray     drop-down menu models array. Array storage TransformGetCacheControllerModel instances
...
@property (nonatomic, assign) CGFloat wildBlueYonderEpisode;
...
@property (nonatomic, assign) CGFloat bareFloat;

/** 15、菜单的弹出的动画效果类型(若不设置，默认为TransformGetCacheControllerViewAnimateType_ScaleBasedTopRight)
 *     -----------------------------------------------------------
 *     English description:
 *     -----------------------------------------------------------
 *     show drop-down menu animate type. (if not assign, default value is TransformGetCacheControllerViewAnimateType_ScaleBasedTopRight)
 */
//: @property (nonatomic, assign) TransformGetCacheControllerViewAnimateType menuAnimateType;
@property (nonatomic, assign) TransformGetCacheControllerViewAnimateType all;

/** 2、cell的类名, 必须是TransformGetCacheControllerBasedCell的子类
 *               若用框架自带的cell,直接传@"TransformGetCacheControllerCell"
 *               若使用自定义的cell,就传自定义cell的类名,若cell是xib,则传@"类名.xib"
 *                     举例如下:
 *                            ----->非xibcell   举例:@"YourCustomMenuCell"
 *                            ----->xib的cell   举例:@"YourCustomMenuCell.xib"
 *
 *               若不设置，默认为 @"TransformGetCacheControllerCell"
 *
 *     -----------------------------------------------------------
 *     English description:
 *     -----------------------------------------------------------
 *     drop-down nemu cell's class name. Must be a subclass of TransformGetCacheControllerBasedCell
 *              If want to use this framework's drop-down nemu cell, direct pass @"TransformGetCacheControllerCell"
...
/** 1、下拉菜单模型数组(数组里面存放 TransformGetCacheControllerBasedModel的子类的对象) 
 *     -----------------------------------------------------------
 *     English description:
 *     -----------------------------------------------------------
 *     drop-down menu models array(Array storage TransformGetCacheControllerModel instances or the TransformGetCacheControllerModel's subclass instances )
 */
//: @property (nonatomic, strong) NSArray *menuModelsArray;
@property (nonatomic, strong) NSArray *titleContendArray;
///==================================================================
///                    创建下拉菜单方式2
///   适用于自定义下拉菜单样式，单个属性进行赋值
///   >>步骤:
///   1、 [TransformGetCacheControllerView alloc] init]
///   2、 对需要赋值的属性进行赋值
...
@property (nonatomic, assign) UIColor *wish;
...
@property (nonatomic, strong) UIColor *seek;
...
@property (nonatomic, assign) NSInteger preservationSize;
...
@property (nonatomic, assign) CGFloat menuInsert;
...
@property (nonatomic, assign) CGFloat lowTide;
...
@property (nonatomic, assign) CGFloat regularPhoto;
...
@property (nonatomic, assign) CGFloat found;
...
@property (nonatomic, assign) CGSize delaySize;
...
@property (nonatomic, strong) UIColor *component;
...
@property (nonatomic, assign) CGSize diamond;
...
@property (nonatomic, assign) CGFloat aLaCarteArrowPinRadius;

/** 18、delegate
 *
 */
//: @property (nonatomic, weak) id<TransformGetCacheControllerViewDelegate> delegate;
@property (nonatomic, weak) id<TransformGetCacheControllerViewDelegate> characterThroughoutted;
...
@property (nonatomic, assign) CGFloat easy;
/** 弹出菜单 */
//: - (void)showMenu;
- (void)menu;

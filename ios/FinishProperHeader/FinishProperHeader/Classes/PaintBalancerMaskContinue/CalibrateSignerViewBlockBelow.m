...
// __CLOSE_PRINT__
//
//  CalibrateSignerViewBlockBelow.m
//  NIM
//
...
//: @implementation CalibrateSignerViewBlockBelow
@implementation CalibrateSignerViewBlockBelow

//: + (NSString *)fileMD5:(NSString *)filepath
+ (NSString *)instructionMd5:(NSString *)filepath
{
    //: NSString *md5 = nil;
    NSString *md5 = nil;
    ...
    if ([[NSFileManager defaultManager] fileExistsAtPath:filepath])
    {
        ...
        unsigned long long fileSize = [self gestureSuggestSize:filepath];
        //: if(fileSize > 200 * 1024 * 1024) {
        if(fileSize > 200 * 1024 * 1024) {
            //: md5 = [self bigFileMD5:filepath];
            md5 = [self trust:filepath];
        //: }else {
        }else {
...
+ (NSString *)trust:(NSString *)filePath
{
...
+ (void)present:(NSString *)filepath managing:(void(^)(NSString *MD5))completion;
{
    ...
    dispatch_block_t block = ^(){
        ...
        NSString *md5 = [self instructionMd5:filepath];
        //: if(completion) {
        if(completion) {
            //: completion(md5);
            completion(md5);
...
+ (unsigned long long)gestureSuggestSize:(NSString *)filepath
{
